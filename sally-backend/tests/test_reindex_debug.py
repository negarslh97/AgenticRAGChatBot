...
async def test_mongodb_connection():
    ...
    try:
        ...
        published = await db.knowledge_base.count_documents({"status": "published"})
        ...
        # نمایش نمونه مقالات
        logger.info("\n📝 Sample articles:")
        async for article in db.knowledge_base.find({"status": "published"}).limit(3):
...
async def test_full_reindex_single_article(mongodb_client, openai_client):
    ...
    try:
        ...
        # یافتن اولین مقاله
        article = await db.knowledge_base.find_one({"status": "published"})
