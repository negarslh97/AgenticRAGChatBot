    ...
    async def generate_rag_response(
        ...
        try:
            ...
            if query_type == "specific":
                max_tokens = 3500  # سوالات خاص: پاسخ جامع (افزایش یافته از 800 به 3500)
            elif query_type == "explanation":
                max_tokens = 2500  # توضیحات: پاسخ کامل (افزایش یافته از 1500 به 2500)
            else:  # general
                max_tokens = 4000  # سوالات عمومی: پاسخ جامع
...
📏 **راهنمای طول و سبک پاسخ:**
   - این یک سوال خاص است - پاسخ باید دقیق، جامع و کامل باشد
   - ابتدا پاسخ مستقیم را ارائه دهید
   - سپس تمام جزئیات، نکات مهم، هشدارها و محدودیت‌ها را به طور کامل توضیح دهید
   - اگر مثال‌های عددی یا موارد خاص در context وجود دارد، حتماً آنها را ذکر کنید
   - از ساختار واضح با عناوین، لیست‌ها و بندهای مشخص استفاده کنید
   - پاسخ می‌تواند چندین پاراگراف باشد تا تمام اطلاعات مرتبط را پوشش دهد
"""
            elif query_type == "explanation":
                response_guide = """
📏 **راهنمای طول و سبک پاسخ:**
   - این یک درخواست توضیح است - پاسخ کامل و جامع با مراحل واضح ارائه دهید
   - ابتدا خلاصه‌ای مختصر بدهید، سپس مراحل/نکات را با شماره‌گذاری توضیح دهید
   - هر مرحله را به طور کامل با جزئیات توضیح دهید
   - پاسخ کل باید 5-8 پاراگراف باشد تا تمام جوانب را پوشش دهد
"""
            else:  # general
                response_guide = """
📏 **راهنمای طول و سبک پاسخ:**
...
پاسخ شما:"""
    ...
    async def generate_rag_response_stream(
        ...
        try:
            ...
            # 🔥 OPTIMIZED: افزایش max_tokens برای پاسخ‌های جامع‌تر به سوالات پیچیده
            if query_type == "specific":
                max_tokens = 2500  # سوالات خاص: پاسخ جامع (افزایش یافته از 800 به 3500)
            elif query_type == "explanation":
                max_tokens = 4000  # توضیحات: پاسخ کامل (افزایش یافته از 1500 به 2500)
            else:  # general
                max_tokens = 5000  # سوالات عمومی: پاسخ جامع
